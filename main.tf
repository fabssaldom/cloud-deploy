...
resource "google_clouddeploy_target" "target" {
  ...
  execution_configs {
    ...
    service_account   = each.value.target_type == "gke" || each.value.target_type == "run" ? "${each.value.exe_config_sa_name}@${var.project}.iam.gserviceaccount.com" : null
...
resource "google_service_account_iam_member" "tri_sa_actas_exe_sa" {
  ...
  service_account_id = "projects/${var.project}/serviceAccounts/${each.value.exe_sa}@${var.project}.iam.gserviceaccount.com"
...
resource "google_service_account_iam_member" "exe_sa_actas_run_svc_sa" {
  ...
  role               = "roles/iam.serviceAccountUser"
  member             = "serviceAccount:${each.value.exe_sa}@${var.project}.iam.gserviceaccount.com"
}
...
resource "google_service_account_iam_member" "exe_sa_actas_exe_sa" {
  ...
  service_account_id = "projects/${each.value.project}/serviceAccounts/${each.value.exe_sa}@${var.project}.iam.gserviceaccount.com"
  role               = "roles/iam.serviceAccountUser"
  member             = "serviceAccount:${each.value.exe_sa}@${var.project}.iam.gserviceaccount.com"
}
